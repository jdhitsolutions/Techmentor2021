...
#TODO: revise to use remoting to gather data via CIM and PSSessions
#TODO: Add VMGroup information

<#
v3.2
fixed Performance counter errors
...
Param(
    ...
    [switch]$Performance,
    [Parameter(HelpMessage = "Specify if you want resource metering in the report.")]
    [switch]$Metering,
    [switch]$NoEventLog
...
$os = Get-CimInstance -ClassName Win32_OperatingSystem -ComputerName $computername -Property Caption, CSDVersion, LastBootUptime, FreePhysicalMemory, FreeVirtualMemory, MaxProcessMemorySize, TotalVirtualMemorySize, TotalVisibleMemorySize

$osdetail = $os | Select-Object @{Name = "OS"; Expression = {$_.caption}},
...
$cs = Get-CimInstance -ClassName Win32_ComputerSystem -ComputerName $computername -Property Manufacturer, Model, TotalPhysicalMemory, NumberOfLogicalProcessors, NumberofProcessors |
...
$vols = Get-Volume -CimSession $computername |
    Where-Object drivetype -eq 'fixed' | Sort-Object -property DriveLetter |
...
$services = Get-CimInstance -ClassName win32_service -filter "name like 'vmi%' or name ='vmms'" -ComputerName $computername -property Name, Displayname, StartMode, State, Startname |
...
if ($runningVMs) {
    $isv = $runningVMS | Sort-object -property IntegrationServicesVersion |
        Select-object -property Name, IntegrationServicesVersion, @{Name = "Current"; Expression = {
            $test = (Hyper-V\Get-VMIntegrationService -VMName $_.Name -ComputerName $_.computername).Where( {$_.OperationalStatus -contains 'ProtocolMismatch'})
...
#$fragments+= "<h3>Replication</h3>"
$repl = Hyper-V\Get-VMReplication -ComputerName $Computername

if ($repl) {
    [xml]$html = $repl |
        Select-Object -property Name, State, Health, Mode, PrimaryServer, ReplicaServer, LastReplicationTime |
    ...
    $fragments += "<p style='color:red;'>No VM replication configured.</p>"
}
...
if ($Metering) {
    ...
    $data = Hyper-V\Measure-VMResourcePool -name * -computer $computername -ErrorAction SilentlyContinue |
        Select-Object -property ResourcePoolname, AvgCPU, AvgRam, MinRam, MaxRam, TotalDisk,
...
if ($Performance) {
    ...
    Try {
        ...
        $data = $diskCounters | Select-Object -ExpandProperty CounterSamples | Where-Object CookedValue -gt 0
    }
    Catch {
    ...
    $hvCounters = Get-Counter -ListSet Hyper-V* -ComputerName $computername -ErrorAction SilentlyContinue
    ...
    if ($hvCounters) {
        ...
        $fragments += "<p style='color:red;'>No Hyper-V performance counters detected</p>"
    }
    $fragments += "</div>"
